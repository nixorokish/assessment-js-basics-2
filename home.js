...
/* 
    Write an *arrow* function called `greetUser` It should have one parameter, `username`
    The function should return a string that says: 'Welcome back, [USERNAME]' Where USERNAME is the `username` arguement sent in

    For example, if called with `Andrew` as the argument, `greetUser` should return the string: 'Welcome back, Andrew'
*/
...
const canWeDeliver = (zipCode) => {
    if (deliveryAreaZipCodes.includes(zipCode) === true) {
        ...
        console.log(`${zipCode}: Sorry, we can't deliver to that address.`)
    }
}
...
/* 
    ...
    If you wrote `canWeDeliver` using a loop of some kind, write a new function (`canWeDeliverTwo`)
    below, using the `includes` array method. Look it up on MDN if you're not sure how to use it. 

    If you already used the `includes` method, write a new function using some sort of 
    loop (for loop, higher order array method). Name your new function `canWeDeliverTwo`.
*/
console.log(`\nProblem 2b:`)
// CODE HERE
canWeDeliverTwo = (zipCode) => {
    for (i = 0; i < deliveryAreaZipCodes.length; i++) {
        if (zipCode === deliveryAreaZipCodes[i] === true) {
            console.log(`${zipCode}: You're in our delivery zone!`)
            break
        } else {
            console.log(`${zipCode}: Sorry, we can't deliver to that address.`)
